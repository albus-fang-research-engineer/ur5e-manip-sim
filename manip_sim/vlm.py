...
def build_emission_prompt(stage: StageSpec, vocab: Vocabulary,
    ...
    system = (
        ...
        "smallest rotation from the entry attitude. The path TSR is the "
        "set of attitudes allowed along the way: put rows on the "
        "directions that must hold throughout the stage. A direction the "
        "stage rotates must be left free on the path — a path row on it "
        "forbids the rotation — and constrained only in the subgoal. "
        "Rule-table limit per TSR: ONE "
        ...
        "plus a perpendicular row is redundant at best (dropped) or "
        "rejected; two perpendicular rows are rejected. "
        "Translation terms are relative to anchor points on the passive "
        "object, in w with z up; 'centered' and 'expr' bound x/y only, "
        "'inside' bounds all three rows. Path terms hold for the WHOLE "
        "motion from entry to subgoal, so the entry pose must satisfy them "
        "(the planner closes at most a 0.03 m gap at the start) and the "
        "path must contain the subgoal; leave rows the stage does not need "
        "free — clearance while moving comes from collision checking.\n\n"
